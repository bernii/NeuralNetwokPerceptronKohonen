...
public class KohonenElements {
    ...
    public static double neighborhood(double distance, double lambda){
        ...
        return Math.exp((-1*Math.pow(distance,2)*Math.pow(2*Math.pow(lambda,2),-1)));
    }
    public static double fatigue(double actualFatigue, boolean isWinner,int neurons_num){
        ...
        if(isWinner){
        	...
        	fatigue = /*Float.parseFloat("0.25")+*/actualFatigue + (float)Math.pow(neurons_num, -1) ;
        }
        if(fatigue<0)
    ...
    private static double weight(double oldWeight, double learningFactor, double neighborhood, double input){
        return oldWeight + learningFactor*neighborhood*(input-oldWeight) ;
    }
    public static void weight(Matrix inputColumn, Matrix weights, int winnerNumber,double learningFactor,double neighborhood){
